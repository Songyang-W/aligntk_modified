/*
 *  register.c  -  registers two grayscale images, producing a map of
...
int initialMapLevel;
int initialMapFactor;
MapElement *initialMap = 0;
int mpwi, mphi;
int offxi, offyi;
            ...
            char **envp)
{
  ...
  for (i = 1; i < argc; ++i)
    ...
    else if (strcmp(argv[i], "-initial_map") == 0)
      {
	if (++i == argc)
	  {
	    error = 1;
	...
	strcpy(c.initialMapName, argv[i]);
      }
  ...
  if (error)
    {
      fprintf(stderr, "Usage: register -images file_prefix -output file_prefix\n");
      ...
      fprintf(stderr, "              [-initial_map <initial_map_prefix>]\n");
  ...
  if (c.imageBasename[0] == '\0' || c.outputMapBasename[0] == '\0' ||
      pairsFile[0] == '\0')
    Error("-images, -output, and -pairs parameters must be specified.\n");
  if (c.cptsMethod < 0)
    c.cptsMethod = AFFINE_METHOD;
...
WorkerTask ()
{
  ...
  char cptsName[PATH_MAX], initialMapName[PATH_MAX], constrainingMapName[PATH_MAX];
  ...
  if (c.cptsName[0] != '\0')
    ...
    cptsName[0] = '\0';
  if (c.initialMapName[0] != '\0')
    sprintf(initialMapName, "%s%s.map", c.initialMapName, t.pair.pairName);
  else
    initialMapName[0] = '\0';
  ...
  if (!computeMap && initialMapName[0] != '\0')
    {
      if (stat(initialMapName, &sb) == 0 &&
	  (double) (sb.st_mtime) > outputTime)
	computeMap = 1;
    }
  ...
  /* get the initial map, if present */
  if (initialMapName[0] != '\0')
    {
      if (!ReadMap(initialMapName,
		   &initialMap,
		   &initialMapLevel,
		   &mpwi, &mphi,
		   &offxi, &offyi,
	...
	Error("Error reading map:\n %s\n", errorMsg);
      initialMapFactor = 1 << initialMapLevel;
      Log("Read initial map %s of size %d x %d;  initialMapFactor = %d\n",
	  initialMapName, mpwi, mphi, initialMapFactor);
    }
  else
    initialMapFactor = 0;
...
Compute (char *outputName, char *outputWarpedName, char *outputCorrelationName)
{
  ...
  int dnx, dny;
  int immbpl;
  unsigned char *initialMapMask;
  float *initialMapDist;
  double lb;
  ...
  else if (initialMapFactor != 0)
    {
      startLevel = 0;
      while ((1 << startLevel) < initialMapFactor)
	++startLevel;
      if ((1 << startLevel) != initialMapFactor)
	{
	  Log("initialMapFactor not a power of 2 -- ignoring initial map\n");
	  initialMapFactor = 0;
	...
	if (initialMapFactor != 0 && nCpts < 2)
	  {
	    ...
	    */
	    ...
	    dnx = dnx * lFactor / initialMapFactor + 1;
	    dny = dny * lFactor / initialMapFactor + 1;
	    immbpl = (dnx + 7) >> 3;
	    initialMapMask = (unsigned char *) malloc(dny * immbpl);
	    memset(initialMapMask, 0, dny * immbpl);
	    for (y = 0; y < mphi; ++y)
	      for (x = 0; x < mpwi; ++x)
		if (initialMap[y*mpwi+x].c > 0.0)
		  {
		    ixv = x + offxi;
		    iyv = y + offyi;
		    if (ixv >= 0 && ixv < dnx &&
			iyv >= 0 && iyv < dny)
		      initialMapMask[iyv*immbpl+(ixv >> 3)] |= 0x80 >> (ixv & 7);
		  }
	    initialMapDist = (float *) malloc(dny * dnx * sizeof(float));
	    computeDistance(CHESSBOARD_DISTANCE, dnx, dny,
			    initialMapMask, initialMapDist);
	      ...
	      for (x = 0; x < mpw; ++x)
		{
		  ...
		  /* lookup in the map */
		  xv = (xv / initialMapFactor) - offxi;
		  yv = (yv / initialMapFactor) - offyi;
		  ixv = (int) floor(xv);
		  iyv = (int) floor(yv);
		  rrx = xv - ixv;
		  rry = yv - iyv;
		  ...
		  if (iyv < 0)
		    ...
		    iyv = dny-1;
		  threshold = initialMapDist[iyv*dnx+ixv] + 2.0;
		  ...
		  if (ixv >= 0 && ixv < mpwi-1 &&
		      iyv >= 0 && iyv < mphi-1)
		    {
		      GETMAP(initialMap, mpwi, ixv, iyv, &rx00, &ry00, &rc00);
		      GETMAP(initialMap, mpwi, ixv, iyv+1, &rx01, &ry01, &rc01);
		      GETMAP(initialMap, mpwi, ixv+1, iyv, &rx10, &ry10, &rc10);
		      GETMAP(initialMap, mpwi, ixv+1, iyv+1, &rx11, &ry11, &rc11);
		      if (rc00 > 0.0 &&
			  rc01 > 0.0 &&
			  rc10 > 0.0 &&
			  rc11 > 0.0)
		      ...
		      else
			{
			  if (!Extrapolate(&rx, &ry, &rc,
					   ixv, iyv, rrx, rry,
					   initialMap, mpwi, mphi, threshold))
		  ...
		  SETMAP(map, mpw, x, y,
			 rx * initialMapFactor / lFactor,
			 ry * initialMapFactor / lFactor, 1.0);
		  Log("Setting map[%d,%d] to (%f %f)\n",
		      x, y,
		      rx * initialMapFactor / lFactor,
		      ry * initialMapFactor / lFactor);
		}
	    free(initialMapDist);
	    free(initialMapMask);
	    free(initialMap);
...
#else
	    txd = fabs(fmod(nomThetaX[k] - thetaX + 3.0*M_PI,
			    2.0*M_PI) - M_PI);
	    tyd = fabs(fmod(nomThetaY[k] - thetaY + 3.0*M_PI,
			    2.0*M_PI) - M_PI);
	    de += txd * txd + tyd * tyd +
	      (l0 - nomL0[k]) * (l0 - nomL0[k]) +
	      (l1 - nomL1[k]) * (l1 - nomL1[k]) +
      ...
      while (moveCount < goalMoveCount)
	{
	  ...
	  newDPoints = dPoints;
		...
		continue;
		  ...
		  ntxd = fabs(fmod(nomThetaX[k] - nThetaX + 3.0*M_PI,
				   2.0*M_PI) - M_PI);
		  ntyd = fabs(fmod(nomThetaY[k] - nThetaY + 3.0*M_PI,
				   2.0*M_PI) - M_PI);
#if 0
		...
		int mpw, int mph, int mox, int moy)
{
  ...
  if (level > c.outputLevel)
    sprintf(fn, "%s.%.2d.map", outputName, level);
  else
		  ...
		  int w, int h, float scale)
{
  ...
  if (level > c.outputLevel)
    sprintf(fn, "%s.%.2d.%s", outputName, level, extension);
  else
