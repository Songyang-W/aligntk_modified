/*
 * inspector.cc  - display pairs of images so that
...
main (int argc, char **argv)
{
  ...
  if (error)
    {
      fprintf(stderr, "Usage: inspector -input file_prefix -map file_prefix\n");
  ...
  //  availableHeight = 925;
  //  availableWidth = 1270;
  window = new MyWindow(0, 0, 256, 256);
  window->loadSection();
  window->reshape();
  window->show();
...
MyWindow::MyWindow (int x, int y, int w, int h) : Fl_Gl_Window (x, y, w, h)
{
...
void
MyWindow::initTextures (int w, int h)
{
...
void
MyWindow::setTexture (unsigned int type, unsigned char *image, int w, int h)
{
  int x, y;
  unsigned char b;
...
bool
MyWindow::backmap (float *wrx, float *wry, float x, float y)
{
  float rx, ry;
...
bool
MyWindow::forwardmap (float *px, float *py, float *pc, float x, float y)
{
  float xv, yv;
  int ixv, iyv;
  int irx, iry;
