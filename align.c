/*
 * align.c  - align a set of maps using a relaxation algorithm
...
main (int argc, char **argv)
{
        ...
        if (p == 0)
        {
                ...
                for (i = 1; i < argc; ++i)
                        ...
                        else if (strcmp(argv[i], "-output_log") == 0)
                        {
                                if (++i == argc)
                                {
                                        error = 1;
                ...
                /* check if output log name was provided */
                if ( outputLogPrefix[0] == '\0' )
                  strcpy( &outputLogPrefix[0], "logs/");
        ...
        if (MPI_Bcast(mapsName, PATH_MAX, MPI_CHAR, 0, MPI_COMM_WORLD) != MPI_SUCCESS ||
            ...
            MPI_Bcast(fontFileName, PATH_MAX, MPI_CHAR, 0, MPI_COMM_WORLD) != MPI_SUCCESS ||
...
Output (int level, int iter)
{
        ...
        for (i = myFirstImage; i <= myLastImage; ++i)
        {
                ...
                if (outputName[0] != '\0')
                {
                        ...
                        if (!WriteMap(fn, map, level, nx, ny, 0, 0, images[i].name, "align",
                                      UncompressedMap, msg))
